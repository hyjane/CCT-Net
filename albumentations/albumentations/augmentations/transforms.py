...
class ToFloat(ImageOnlyTransform):
    ...
    If `max_value` is None the transform will try to infer the maximum value by inspecting the datas type of the input
    image.
...
class FromFloat(ImageOnlyTransform):
    ...
    the maximum value for the datas type from the `dtype` argument.

    This is the inverse transform for :class:`~albumentations.augmentations.transforms.ToFloat`.

    Args:
        max_value (float): maximum possible input value. Default: None.
        dtype (string or numpy datas type): datas type of the output. See the `'Data types' page from the NumPy docs`_.
            Default: 'uint16'.
        p (float): probability of applying the transform. Default: 1.0.
        ...
        p_replace (float or tuple of float): Defines for any segment the probability that the pixels within that
            ...
            Examples:
                ...
                * A probability of ``1.0`` would mean, that all segments are
                  ...
                  image).
            Behaviour based on chosen datas types for this parameter:
